...
from tkinter import Tk  # Import Tk từ thư viện tkinter để tạo cửa sổ ứng dụng

# Lớp Controller để điều khiển luồng dữ liệu giữa Model (Database) và View (LoginView, MainView)
class Controller:
    def __init__(self):
        # Khởi tạo đối tượng Database để kết nối và thao tác với cơ sở dữ liệu
        self.database = Database()
        
        # Khởi tạo cửa sổ chính của ứng dụng
        self.root = Tk()
        
        # Hiển thị giao diện đăng nhập đầu tiên
        self.show_login_view()
        
        # Bắt đầu vòng lặp chính của tkinter để ứng dụng có thể chạy và hiển thị
        self.root.mainloop()
    ...
    # Các phương thức bổ sung cho Controller để xử lý các hành động từ giao diện có thể được thêm vào đây

# Đoạn mã này kiểm tra nếu tập tin được chạy trực tiếp, Controller sẽ được khởi chạy
